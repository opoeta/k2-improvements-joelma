...
- **Probe & Z-offset** — passos de **0.005 a 0.1 mm**, **Ler valor atual** (offset aplicado + z_offset salvo no probe), **teste de 1ª camada** (quadrado sólido — perímetro + preenchimento, baseado no gcode de Z-offset do Creality Print) pra julgar o squish, `Z_OFFSET_APPLY_PROBE` + `SAVE_CONFIG` e `PROBE_ACCURACY`. **Z-offset persistente por placa + material**: grava o offset calibrado pra combinação (ex. PLA + textured) via `SAVE_VARIABLE` e o `START_PRINT` aplica sozinho a cada impressão — sobrevive a restart (resolve o "offset zera quando reinicia").
